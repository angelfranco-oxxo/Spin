// Dashboard SPIN BY OXXO · Gestión Promocional · Plaza Oaxaca
// Datos en vivo desde dashboard3-sheet.js. Gráficos: SVG a mano, sin librerías
// (mismo patrón que premia.js, adaptado a promociones en vez de métricas fijas).
'use strict';
...
function barChartSVG(items, { value, max, suffix = '%' }) {
  ...
  const bars = items.map((it, i) => {
    ...
    const nombre = it.asesor.split(' ')[0];
...
let FILAS = [], PROMOS = [], PERIODO = '';
const $ = id => document.getElementById(id);
const kpisEl = $('kpis'), tabsEl = $('promoTabs'), chartEl = $('chart'), listEl = $('alist'), emptyEl = $('empty');
...
loadPromoData()
  .then(({ FILAS: f, PROMOS: p, PERIODO: per }) => { FILAS = f; PROMOS = p; PERIODO = per; init(); })
  .catch(err => {
...
function tiendasDePromo(p) { return FILAS.filter(f => f.promo === p); }

function asesoresDePromo(p) {
  const rows = tiendasDePromo(p);
  const by = {};
  rows.forEach(r => {
    if (!by[r.asesor]) by[r.asesor] = { asesor: r.asesor, tiendas: 0, uds: 0, meta: 0, faltante: 0, sumAvance: 0 };
    const a = by[r.asesor];
    a.tiendas++; a.uds += r.uds; a.meta += r.meta; a.faltante += r.faltante; a.sumAvance += r.avance;
  });
...
function rowHTML(d, i, isAsesor) {
  return `
    <div class="arow${i < 3 ? ' top' + (i + 1) : ''}${isAsesor ? ' clicable' : ''}"${isAsesor ? ` data-asesor="${d.asesor.replace(/"/g, '&quot;')}"` : ''}>
      ...
      <div class="acnt">${nf(d.uds)}/${nf(d.meta)}</div>
    </div>`;
}

function init() {
  tabsEl.innerHTML = PROMOS.map((p, i) => `<button class="tab mtab${i === 0 ? ' on' : ''}" data-p="${p.replace(/"/g, '&quot;')}">${p}</button>`).join('');
  promo = PROMOS[0] || '';
  tabsEl.querySelectorAll('.mtab').forEach(tb => tb.addEventListener('click', () => {
    if (tb.dataset.p === promo) return;
...
function render() {
  const tiendas = tiendasDePromo(promo);
  const asesores = asesoresDePromo(promo);
  ...
  chartEl.innerHTML = barChartSVG(ranked, { value: a => a.avance, max: 100, suffix: '%' });
  ...
  distHintEl.textContent = `Reparto de las ${tiendas.length} tiendas para ${promo}.`;
...
function renderModal() {
  const tiendas = tiendasDePromo(promo).filter(t => t.asesor === modalAsesor &&
    (!modalTexto || `${t.tienda} ${t.cr}`.toLowerCase().includes(modalTexto)));
  const sorted = [...tiendas].sort((a, b) => b.avance - a.avance);
  modalSub.textContent = `${tiendas.length} tiendas · promoción: ${promo}`;
